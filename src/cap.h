...
enum cap_type {
CAP_TYPE_EMPTY, CAP_TYPE_MEMORY, CAP_TYPE_PMP, CAP_TYPE_TIME, CAP_TYPE_CHANNELS, CAP_TYPE_ENDPOINT, CAP_TYPE_SUPERVISOR
};
...
static inline struct cap cap_mk_memory(uint64_t begin, uint64_t end, uint64_t rwx, uint64_t free, uint64_t pmp) {
...
static inline struct cap cap_mk_time(uint64_t hartid, uint64_t pid, uint64_t begin, uint64_t end, uint64_t free, uint64_t depth) {
...
static inline struct cap cap_mk_channels(uint64_t begin, uint64_t end, uint64_t free, uint64_t ep) {
kassert((begin & 0xffffull) == begin);
kassert((end & 0xffffull) == end);
kassert((free & 0xffffull) == free);
...
struct cap c;
c.word0 = (uint64_t)CAP_TYPE_ENDPOINT << 56;
c.word1 = 0;
c.word0 |= mode;
c.word0 |= channel << 8;
return c;
...
kassert((cap.word0 >> 56) == CAP_TYPE_ENDPOINT);
return (cap.word0 >> 8) & 0xffffull;
}
...
static inline struct cap cap_mk_supervisor(uint64_t begin, uint64_t end, uint64_t free) {
...
return b;}
if (parent_type == CAP_TYPE_CHANNELS && child_type == CAP_TYPE_ENDPOINT) {
